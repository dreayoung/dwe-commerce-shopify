  ...
  return (
    <div className="flex flex-col space-y-20 py-24">
      <div className="relative flex h-40 items-center justify-center overflow-x-hidden text-center">
        ...
        <span className="absolute left-1/2 top-auto hidden -translate-x-1/2 transform whitespace-nowrap text-center font-vcr text-5xl uppercase text-black/5 md:block md:text-8xl lg:text-[7rem]">
          Herotoall.io
        </span>
        <span className="words absolute top-auto block text-center font-vcr text-8xl uppercase text-black/5 md:hidden">
          Herotoall.io
        </span>
      </div>
      <div className="px-4 md:px-20">
        <div className="carousel mx-auto flex w-full space-x-4 rounded-2xl bg-black/5 p-4">
          {images.map((url, x) => {
            return (
              <div
                className="relative box-content flex aspect-square h-[30rem] w-72 flex-none animate-carousel snap-center md:w-96"
                key={x}
              >
                <Image
                  src={url}
